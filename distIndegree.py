...
class Indegree():
    ...
    def process_all(self, minid, maxid, run_counter):
        try:
            for id in range(minid, maxid + 1):
                ...
                self.indeg(fpath)
                self.current_id = id
                print(10 * '=', ' - distIndegree.py - run # %d - ' % run_counter, 10 * '=')
            self.process()
            fname = 'indegree_id%d-%d_rc%d.csv' % (minid, maxid, run_counter)
            self.write_out(fname)
        except KeyboardInterrupt:
            print('saving values..')
            self.process()
            fname = 'indegree_id%d-%d_rc%d.csv' % (minid, self.current_id, run_counter)
            self.write_out(fname)
            sys.exit(0)
    ...
    def indeg(self, fpath):
        net = nx.read_graphml(fpath)
        odegree = net.in_degree()
...
if __name__ == "__main__":
    ...
    def read_counter():
        return loads(open("counter_indegrees.json", "r").read()) + 1 if os.path.exists("counter_indegrees.json") else 0

    def write_counter():
        with open("counter_indegrees.json", "w") as f:
            f.write(dumps(run_counter))

    run_counter = read_counter()
    atexit.register(write_counter)

    proc = Indegree(indir, outdir)
